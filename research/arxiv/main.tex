...
\newcommand{\snapshot}{\texttt{542cd5c01a79942ecb78040b19deb4e82f4f9068}}
...
semantic engine with compatibility tooling and a Tauri 2 Workbench. The stable public
boundary is 3.0.2; this paper also describes the reconciled development snapshot
\snapshot, which contains active graph and launch-readiness work and must not be read as a
claim about the stable release. Comparative cross-model benchmark results remain pending.
\end{abstract}
...
The stable public CLI includes \texttt{init}, \texttt{adopt}, \texttt{import-plan},
\texttt{new}, \texttt{validate}, \texttt{dashboard}, \texttt{spec},
\texttt{check-frozen}, and \texttt{doctor}. The development \mainbranch{} additionally
exposes newer operational surfaces including \texttt{verify ci}, grouped
\texttt{release} verification/build/inspect commands, and graph operations. Their
presence identifies the current implementation boundary; it does not imply that stable
3.0.2 has the same surface or that every release gate has closed.
...
Windows, Linux, and Android bring-up evidence exists. macOS and iOS remain intended
targets and require native platform-specific validation. A current Windows development
snapshot is shown in Figure~\ref{fig:workbench}; it is intentionally labelled as
development state and makes no stable-release claim.
...
\begin{figure}[t]
  ...
  work-item surface at the reconciled development snapshot. This is an implementation
...
WI046 established the local-first verification and release architecture in the development
line. Local validation, evidence, release inspection, and build checks are now durable
development architecture rather than future work. That architecture still does not make
all external admission gates green. In particular, local verification success is not remote
enforcement closure, and protected admission remains incomplete.
...
the driver, scoring, and harness infrastructure exist. Deterministic mock runs validate
plumbing, not agent behavior, and no comparative cross-model results are claimed. S8's
secondary cost metrics include time, tokens, tool calls, file reads, bytes read, and
repository-wide search operations.
...
\bottomrule
\end{tabular}
\caption{Current findings through the reconciled development snapshot. The grouped rows
...
The current development implementation provides an active, derived, versioned Repository
Orientation Graph substrate with deterministic rebuild, incremental convergence,
language-aware semantic extraction, working-tree overlays, and explicit freshness and
coverage state.

The graph remains non-authoritative: source and governance records remain the source of
truth. Higher-level orientation queries, adoption integration, broader topology,
performance closeout, operator-facing graph workflows, and the graph-enabled S8 evaluation
remain incomplete. Any graph-enabled condition must be added through a dated amendment and
measured against the pre-registered no-graph baseline rather than assumed valuable from
...
cases held. The enforcement-closure observation motivated a hypothesis rather than being
treated as proof. Governance continuity and graph-enabled orientation are registered
prospective work, not empirical results.

Future work includes real PactBench and comparative runs; enforcement-closure and S8 studies
outside ForgeWire-controlled repositories; third-party reproduction; maintaining
operator-control parity as orientation, verification, and admission capabilities are added;
native macOS and iOS validation; provenance-preserving external ingestion; higher-level
graph queries and adoption integration; performance closeout; stronger temporal and
relational invariants; and independent conformance implementations.

\section*{Reproducibility and artifact availability}

This submission is frozen against RepoPact development snapshot:
\begin{quote}\texttt{\scriptsize\snapshot}\end{quote}
The stable
software boundary is 3.0.2; development-only commands and graph work are labelled as such.
The repository contains the formal model, benchmark protocol, threats and findings
registers, conformance suite, evidence records, Workbench source, and launch-readiness
records. The arXiv package is a clean source package under \texttt{research/arxiv}; its
