...
| F-011 | H7 | major | An older adopter (ForgeLink) drifted *invalid* as the standard evolved (stale registry paths, missing root contract); nothing detected or guided the upgrade | [009](captures/009-forgelink-upgrade.md) | reconciled manually on a branch; a `repopact upgrade`/`doctor` is the open need |
