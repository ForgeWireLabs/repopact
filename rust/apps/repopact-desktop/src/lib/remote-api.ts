...
// narrow `remote_*` Tauri command (Decision 0061) -- there is no
// `github_request`/`provider_request`/`authenticated_fetch` command, and
// none should ever be added here. This module never calls `fetch()`
// against GitHub itself; all network I/O happens natively.
